...
In this image, the Brandenburg Gate location is in black, the satellite path is in magenta, and the river is in blue. Note that since no information is given about the satellite at all (its altitude, whether its in e.g., LEO or GEO orbit, etc), I had to assume its a straight line path. Over such a short distance this is probably pretty accurate anyway.
...
As a sanity check I did plot these results using `plot_distribs()`, but you'll see it's commented out as it's not needed for the solution. This will output the below image.
...
Now to calculate the probability distribution functions for each of the sources. I came across the [`shapely`](https://github.com/Toblerity/Shapely) package on StackOverflow for analysis in cartesian geometry, which I used for calculating the distance from points to points/lines. However, its worth noting other ways to this. See `distance.py` for some functions to use maths to calculate the same thing. Note the `haversine()` function there (which I also found on Stackoverflow) can be used to do this in spherical geometry instead!
...
![3D view of the distributions](http://i.imgur.com/VWcemi5.png)

![3D view of the total probability distribution](http://i.imgur.com/9lu22Oy.png)
